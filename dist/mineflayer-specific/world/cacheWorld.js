...
class BlockInfo {
    ...
    static fromBlock(b) {
        ...
        if (b.boundingBox === 'block') {
            ...
            return new BlockInfo(false, false, true, true, false, BlockInfo.liquids.has(b.type) || Boolean((_a = b._properties) === null || _a === void 0 ? void 0 : _a.waterlogged), false, b.position.y, false, b.position, b.type, b);
        }
    }
    static SOLID(pos) {
...
BlockInfo.substituteBlockStateId = 1;
class CacheSyncWorld {
    constructor(bot, referenceWorld) {
        this.cacheCalls = 0;
        this.enabled = true;
        this.minY = bot.game.minY;
        this.posCache = new lru_cache_1.LRUCache({ max: 10000, ttl: 2000 });
        this.posCache1 = new lru_cache_1.LRUCache({ max: 10000, ttl: 2000 });
        this.blocks = new lru_cache_1.LRUCache({ size: 10000, max: 2000 });
        ...
        this.world.getBlock = utils_1.fasterGetBlock.bind(this.world);
    }
    makeLRUCache(size) {
    ...
    getBlock(pos) {
        ...
        if (this.posCache.has(key))
            return this.posCache.get(key);
        const block = this.world.getBlock(pos);
        if (block !== null)
            this.posCache.set(key, block);
        return block;
    }
    getBlockInfo(pos) {
        if (!this.enabled) {
    ...
    getBlockStateId(pos) {
        ...
        const key = `${pos.x}:${pos.y}:${pos.z}`;
        if (this.posCache.has(key))
            return this.posCache1.get(key);
        const state = this.world.getBlockStateId(pos);
        if (state !== undefined)
            this.posCache1.set(key, state);
        return state;
    }
    getCacheSize() {
